...
function RemindersPopupModal({ currentYear }: { activeDate: string }) {
  ...
  return (
    <div className="modal">
      <div
        ...
        className="overlay bg-backgroundemphasis fixed bottom-0 left-0 right-0 top-0 z-30 h-full  w-full opacity-60"></div>
      <div className=" modal_content absolute bottom-0 z-40 w-full rounded-t-lg border bg-white px-5 pb-8  text-gray-900 md:left-1/2  md:right-1/2   md:flex md:w-1/2 md:flex-col">
        ...
        <div className="modal-body">
          <form
            ...
            <h1 className="mb-2 mt-2 font-bold text-gray-800">Title</h1>
            <input
              type="text"
              required
              onChange={(e) => setReminderTitle(e.target.value)}
              placeholder="New Reminder"
              className="h-[40px] w-full rounded-md  bg-gray-200 px-2 text-gray-400"
            />
            <div className="buttons  bottom-0 flex gap-2">
              <button
                className="mt-8 w-full
              rounded-md border border-indigo-600 border-transparent bg-gray-100 px-4 py-2 text-sm font-medium text-indigo-600 shadow hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2  "
                onClick={() => setOpenModel(false)}>
                Close
              </button>
              <button
